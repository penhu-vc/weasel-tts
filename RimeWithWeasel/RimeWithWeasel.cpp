...
RimeWithWeaselHandler::RimeWithWeaselHandler(UI* ui)
    : m_ui(ui),
      ...
      m_koreanTTS(std::make_unique<KoreanTTSHandler>()) {
  m_ui->InServer() = true;
...
void RimeWithWeaselHandler::Initialize() {
  ...
  // Initialize Korean TTS with user data path
  if (m_koreanTTS) {
    std::wstring userDataPath = WeaselUserDataPath().wstring();
    m_koreanTTS->Initialize(userDataPath);
  }
...
BOOL RimeWithWeaselHandler::ProcessKeyEvent(KeyEvent keyEvent,
  ...
  // Korean TTS: Track backspace for blacklist mechanism
  if (m_koreanTTS && !(keyEvent.mask & ibus::Modifier::RELEASE_MASK)) {
    RIME_STRUCT(RimeContext, ctx);
    bool hasComposition = false;
    if (rime_api->get_context(session_id, &ctx)) {
      hasComposition = ctx.composition.length > 0;
      rime_api->free_context(&ctx);
    }

    // Backspace key (VK_BACK = 0xff08 in ibus keycode)
    if (keyEvent.keycode == 0xff08) {
      m_koreanTTS->OnBackspace(hasComposition);
    } else {
      m_koreanTTS->ResetBackspaceTracking();
    }

    // Check clipboard on Ctrl+V
    if ((keyEvent.mask & ibus::Modifier::CONTROL_MASK) &&
        (keyEvent.keycode == 'v' || keyEvent.keycode == 'V')) {
      m_koreanTTS->CheckClipboard();
    }
  }
...
bool RimeWithWeaselHandler::_Respond(WeaselSessionId ipc_id, EatLine eat) {
  ...
  if (rime_api->get_commit(session_id, &commit)) {
    ...
    // Korean TTS: Record commit for history tracking
    if (m_koreanTTS) {
      m_koreanTTS->OnCommit(u8tow(commit.text));
    }
...
void RimeWithWeaselHandler::_GetContext(Context& weasel_context,
  ...
  if (rime_api->get_context(session_id, &ctx)) {
    if (ctx.composition.length > 0) {
      ...
      // Korean TTS: Trigger pronunciation for Korean input
      if (m_koreanTTS) {
        RIME_STRUCT(RimeStatus, status);
        if (rime_api->get_status(session_id, &status)) {
          std::wstring schemaId = status.schema_id ? u8tow(status.schema_id) : L"";
          m_koreanTTS->OnPreeditUpdate(weasel_context.preedit.str, schemaId);
          rime_api->free_status(&status);
        }
      }
    }
